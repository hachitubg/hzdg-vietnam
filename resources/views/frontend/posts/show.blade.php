    ...
    <div class="container">
        <div style="max-width:900px;margin:0 auto;">
            ...
            <div style="color:#888;margin-bottom:20px;">
                ...
                @if($post->published_at)<span style="margin-left:15px;">📅 {{ $post->published_at->format('d/m/Y') }}</span>@endif
                <span style="margin-left:15px;">👁️ {{ number_format($post->view_count) }} lượt xem</span>
            </div>
            @if($post->thumbnail)
            <img src="{{ asset('storage/'.$post->thumbnail) }}" alt="{{ $post->title }}" style="width:100%;border-radius:8px;margin-bottom:20px;">
            @endif
        ...
        <div style="margin-top:40px;max-width:900px;margin-left:auto;margin-right:auto;">
            ...
            <div class="row">
                ...
                <div class="col large-6" style="margin-bottom:15px;">
                    <a href="{{ route('posts.show', $rp->slug) }}" style="display:flex;gap:15px;align-items:center;">
                        @if($rp->thumbnail)
