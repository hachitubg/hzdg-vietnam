        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px;">
            @forelse ($posts as $post)
                <article style="border:1px solid #eee;border-radius:8px;padding:12px;">
                    @if ($post->thumbnail)
                        <a href="{{ route('posts.show', $post->slug) }}"><img src="{{ asset('storage/' . $post->thumbnail) }}" alt="{{ $post->title }}" style="width:100%;height:180px;object-fit:cover;border-radius:6px;"></a>
                    @endif
                    <h3 style="margin:10px 0;"><a href="{{ route('posts.show', $post->slug) }}">{{ $post->title }}</a></h3>
                    <p>{{ $post->excerpt }}</p>
                </article>
            @empty
